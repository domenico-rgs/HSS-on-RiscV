...
int main(int argc, char *argv[]){
    ...
    int8_t y[N];
    int8_t test_data[N_FEATURES*N];
    int i = 0; //total samples processed since turned on
        
    while(1){
        uart_readData(uart0,(uint8_t*)test_data,sizeof(int8_t)*N_FEATURES*N); //it waits if no data are available
        ...
        //uart_writeData(uart0,(uint8_t*)y,sizeof(int16_t)*N_FEATURES*N); //to be used to obtain the output of the network

        for (int wait=0; wait<(3200000/32); wait++) { //wait 0.1s before sending signal for the next sample, increase (e.g., to 0.5) in case data writing by producer
            asm volatile("nop");
        }
